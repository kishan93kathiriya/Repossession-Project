...
public class AttachmentDAO {
	...
	public void delete(AttachmentVO VO_obj) {
		...
		try {
			...
			/*
			 * Method 1 Query q = session.createQuery(
			 * "from AttachmentVO where dID = :dID "); q.setParameter("dID",
			 * VO_obj.getdID()); AttachmentVO dvo =
			 * (AttachmentVO)q.list().get(0);
			 */
	...
	public List search(AttachmentVO VO_obj) {
		...
		try {
			...
			Query q = session
					.createQuery("FROM AttachmentVO WHERE attachmentType like '%" + VO_obj.getAttachmentType() + "%' ");
